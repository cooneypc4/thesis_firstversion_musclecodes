...
Sam (Cheng) Qian, 3/2017;
revised, Patricia Cooney, 4/2020;
Grueber Lab
Columbia University
...
for n=1:numFiles;
    ...
    %preallocate CIthresh pts
    CIthreshpts = nan(1,numFrames);
...
%         end
        ...
        if size(B,1)~=0;
            ...
            for p=1:300;
                ...
                %find number of points with CI over threshold 0.75
                dispnum = numel(find(C_radius > 0.05));
                text(50,50,['CIthreshpts:' num2str(dispnum)],'Color', 'w');
                colormap(jet);
                ...
                %save num of points over CI threshold at each time point into excel file
                CIthreshpts(1,i) = dispnum;
    ...
    csvwrite('CIthreshpts',CIthreshpts);
    save(strcat(filename, '_CIthreshpts'),'CIthreshpts');  
end
